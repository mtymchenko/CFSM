...
f0 = 1*GHz; % pulse center frequency
duration = 1*ns;
t0 = 3*ns;
t = linspace(0, 10*ns, pow2(12));

v = exp(-(t-t0).^2/duration^2/2).*exp(1j*2*pi*f0*t);
Vn = fft(v)/length(v);
df = 1/max(t);
freq = [0:df:3*GHz];
Vn = Vn(1:numel(freq));
...
apply_voltage_spectrum(crt, 'SWITCHED_CAP', 1, Vn(1:numel(freq)));
...
subplot(2,1,1)
plot_current(crt, 'SWITCH1', 1, t, 'XUnits', 'ns', 'YUnits', 'mA');
title('CAP current')
grid on
subplot(2,1,2)
plot_resistance(crt, 'SWITCH1', t, 'XUnits', 'ns', 'YUnits', 'Mohm'); hold on
plot_resistance(crt, 'SWITCH2', t, 'XUnits', 'ns', 'YUnits', 'Mohm'); hold off
grid on


figure('Name','CAP voltage')
subplot(2,1,1)
plot_voltage(crt, 'CAP', 1, t, 'XUnits', 'ns', 'YUnits', 'V'); hold on
...
figure
subplot(2,1,1)
plot_stored_energy(crt, 'CAP', 1, t, 'XUnits', 'ns', 'YUnits', 'pJ')
title('CAP')
grid on
subplot(2,1,2)
plot_resistance(crt, 'SWITCH1', t, 'XUnits', 'ns', 'YUnits', 'Mohm'); hold on
