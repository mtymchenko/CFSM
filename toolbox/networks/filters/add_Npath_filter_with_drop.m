...
function add_Npath_filter_with_drop(crt, name, C, N_paths, t_delay)
...
for i = 1:N_paths
    ...
    switch3_name = ['SW_DROP',num2str(i),'_',name];
    ...
    add_resistor(crt, switch3_name, R_min + R_max*( 1-pulstran(t-(i-1)*w-2*w-td,D,'rectpuls',w)) );
    
    make_shunt_T(crt, ['SHUNT_DROP_SWITCH',num2str(i),'_',name], {switch3_name});
    
    paths_names{i} = ['PATH',num2str(i),'_',name];
    
    connect_in_series(crt, paths_names{i}, {switch1_name, ['SHUNT_',cap_name], ['SHUNT_DROP_SWITCH',num2str(i),'_',name], switch2_name});
%     connect_in_series(crt, paths_names{i}, {switch1_name, cap_name, ['SHUNT_DROP_SWITCH',num2str(i),'_',name], switch2_name});
end
