...
function plt = plot_voltage_spectrum(crt, compid, port, varargin)

p = inputParser;
addOptional(p, 'XUnits', 'Hz', @(x) any(validatestring(x, {'Hz','kHz','MHz','GHz','THz','PHz','harmonics'})))
addOptional(p, 'YUnits', 'V', @(x) any(validatestring(x, {'fV','pV','nV','uV','mV','V','kV','MV','GV'})))
...
Y = abs(voltage_spectrum(:,freq_id))/y_unit_factor;
...
title(['Voltage spectrum, V_{',num2str(port),'n}'])
